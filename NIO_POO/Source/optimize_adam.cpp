...
void Optimize_adam::voxels_eud(Plan plan, int rid, int pid, double *voxels) {
...
double Optimize_adam::penalty(Plan plan, unsigned int pid) {
    double penalty = 0;
...
double Optimize_adam::objective(Plan plan, unsigned int pid) {
    double objective = 1;
...
void Optimize_adam::vector_stats(const char *name, double *vector, int n_values) {
...
void Optimize_adam::reduce_gradient(double *voxels, int n_voxels, int n_gradients, int n_plans) {
...
void Optimize_adam::adam(double *gradient, double *momentum, double *variance, int n_beamlets, float step, double *fluence, int n_plans, int t) {
    ...
    for (int i = 0; i < n_plans*n_beamlets; i++) {
        ...
        // Actualizar momento del primer orden (m_1)
        momentum[i] = beta1*momentum[i] + (1-beta1)*gradient[i];
        // Actualizar momento del segundo orden (m_2)
...
int Optimize_adam::descend(Plan plan, double *voxels, double *gradient, double *momentum, double *variance, float step, int t) {
...
void Optimize_adam::optimize(Plan plan) {
    ...
    while (running && get_time_s() - start_time < 600000) {
        ...
        descend(plan, voxels, gradient, momentum, variance, step, t);
